...
static void log_msg(const char* msg) {
    ...
    if (g_log) { fprintf(g_log, "[%llu] %s\n", GetTickCount64(), msg); fflush(g_log); }
}
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE, LPSTR, int) {
    log_msg("Test app starting (preload mode)");

    // Preload the DLL before creating D3D11
    HMODULE vr_dll = LoadLibraryA("vr_converter.dll");
    if (vr_dll) {
        log_msg("Preloaded vr_converter.dll");
    } else {
        log_msg("Failed to preload vr_converter.dll");
    }
